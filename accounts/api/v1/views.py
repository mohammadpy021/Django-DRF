...
class RegistrationView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            mail = EmailMessage(
                "Subject here", f"{token}", "from@gmail.com", [self.email]
            )
...
class LogoutToken(views.APIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                {"detail": ("no token has been setted for the user yet")},
...
class EmailVerificationView(views.APIView):
    def get(self, request, token, *args, **kwargs):
        try:
            ...
            )  #'token' is defiend in the url
        except ExpiredSignatureError:
            return Response(
                "token has been expired", status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if user.is_verified:
            return Response(
                "your account is already verified", status=status.HTTP_200_OK
            )
        user.is_verified = True
        user.save()
        return Response(
            "your account has been verified successfully", status=status.HTTP_200_OK
        )
...
class EmailVerificationResendView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        ):  # "is_verified" is a custom validation defiend in custom user model
            return Response(
